...
cost_image = iio.imread("map.png", mode="L")
print(cost_image.shape)
print(cost_image.dtype)
...
model = GeNNModel("float", "wave_front")
model.dt = 1.0

# Create population of neurons
num_neurons = cost_image.shape[0] * cost_image.shape[1]

lif_params = {"C": 1.0, "TauM": 20.0, "Vrest": -65.0, "Vreset": -65.0, "Vthresh" : -50.0,
              "Ioffset": 0.0, "TauRefrac": 20.0}
lif_init = {"V": -65.0, "RefracTime": 0.0}
neurons = model.add_neuron_population("Neurons", num_neurons, "LIF", lif_params, lif_init)
...
pre_ind, post_ind, weight, delay = zip(*conn_list)

proj = model.add_synapse_population("Synapses", "SPARSE",
                                    neurons, neurons,
                                    init_weight_update("StaticPulseDendriticDelay", {},
                                                       {"g": weight, "d": delay}), 
                                    init_postsynaptic("ExpCurr", {"tau": 5.0}))
proj.set_sparse_connections(pre_ind, post_ind)
proj.max_dendritic_delay_timesteps = 8


# Stimulate stim neuron
stim = model.add_neuron_population("stim", 1, "SpikeSourceArray", {}, 
                                   {"startSpike": 0, "endSpike": 1})
stim.extra_global_params["spikeTimes"].set_init_values([2.0])

stim_proj = model.add_synapse_population("StimSynapses", "SPARSE",
                                         stim, neurons,
                                         init_weight_update("StaticPulseConstantWeight", {"g": instant_spike_weight}),
                                         init_postsynaptic("ExpCurr", {"tau": 5.0}))
stim_proj.set_sparse_connections([0], [get_neuron_index(stim_x, stim_y, cost_image.shape[0])])

model.build()
model.load(num_recording_timesteps=duration)
...
# Read data
spike_times, spike_ids = neurons.spike_recording_data[0]

# Calculate x and y coordinates corresponding to each spike
neuron_x = spike_ids % cost_image.shape[1]
neuron_y = spike_ids // cost_image.shape[1]

# Get time of last spike
end_time = int(np.amax(spike_times))

# Convert spike times to 3D spike matrix
matrix, _ = np.histogramdd((neuron_y, neuron_x, spike_times),
